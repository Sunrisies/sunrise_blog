...
export class GithubCommitController {
  ...
  @ApiQuery({ name: 'repository', required: false, description: '仓库名称', type: String, default: 'nestjs/nest' })
  @ApiQuery({ name: 'branch', required: false, description: '分支名称', type: String })
  @ApiOkResponse({ description: '成功响应', type: PaginatedResponseDto<GithubCommit> })
  async getCommits(
    @Query('page') page = 1,
    ...
    return await this.githubCommitService.getCommits(page, limit, { repository, branch });
  }
