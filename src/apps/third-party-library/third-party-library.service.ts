  ...
  async findAll(
    ...
    try {
      const queryBuilder = this.libraryRepository
        ...
        .select(['library', 'category.id', 'category.name', 'tags.id', 'tags.name'])
  ...
  async update(
    ...
    try {
      ...
      if (updateThirdPartyLibraryDto.tagIds) {
        existingLibrary.tags = await this.tagRepository.findByIds(updateThirdPartyLibraryDto.tagIds)
      }
