...
export class MessageController {
  ...
  @ApiOkResponse({ description: '添加成功', type: ResponseDto<CreateMessageDto> })
  @ApiBody({
    description: '留言信息',
    type: CreateMessageDto,
  })
  @Post()
  async create(@Body() createMessageDto: CreateMessageDto): Promise<ResponseDto<CreateMessageDto>> {
    return await this.messageService.create(createMessageDto);
  }

  @ApiOperation({ summary: '获取留言列表' })
  @ApiOkResponse({ description: '获取成功', type: PaginatedResponseDto<IMessage> })
  @ApiQuery({
    name: 'page',
    description: '页码',
    default: 1, // 默认值为1
